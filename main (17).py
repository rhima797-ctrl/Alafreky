...
# ─── NoorPlay session & search ───────────────────────────────────────────────

_noor_session = http_requests.Session()
_noor_session_lock = threading.Lock()
...
_NOOR_HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
    "Accept-Language": "ar,en;q=0.9",
    "Referer": "https://www.noorplay.com/ar",
    "Accept": "application/json, text/javascript, */*; q=0.01",
    "X-Requested-With": "XMLHttpRequest",
}

NOOR_COOKIES_PATH = "noor_session.json"


def _noor_save_cookies():
    """Persist current NoorPlay session cookies to disk."""
    try:
        cookies_dict = {c.name: c.value for c in _noor_session.cookies}
        with open(NOOR_COOKIES_PATH, "w", encoding="utf-8") as f:
            import json as _json
            _json.dump(cookies_dict, f)
        print("[noorplay] cookies saved")
    except Exception as e:
        print(f"[noorplay] save cookies error: {e}")


def _noor_load_cookies():
    """Load saved NoorPlay session cookies from disk."""
    global _noor_logged_in, _noor_session_ready
    try:
        if not os.path.exists(NOOR_COOKIES_PATH):
            return False
        import json as _json
        with open(NOOR_COOKIES_PATH, "r", encoding="utf-8") as f:
            cookies_dict = _json.load(f)
        if not cookies_dict:
            return False
        for name, value in cookies_dict.items():
            _noor_session.cookies.set(name, value, domain=".noorplay.com")
        print(f"[noorplay] cookies loaded ({len(cookies_dict)} cookies)")
        return True
    except Exception as e:
        print(f"[noorplay] load cookies error: {e}")
        return False


def _noor_verify_session():
    """Check if current session is still valid (user logged in) by checking the homepage."""
    global _noor_logged_in
    try:
        r = _noor_session.get(
            "https://www.noorplay.com/ar",
            headers={"User-Agent": _NOOR_HEADERS["User-Agent"], "Accept-Language": "ar,en;q=0.9"},
            timeout=15,
        )
        _noor_logged_in = "logout" in r.text.lower() or "تسجيل الخروج" in r.text
        print(f"[noorplay] session verified → logged_in={_noor_logged_in}")
        return _noor_logged_in
    except Exception as e:
        print(f"[noorplay] verify session error: {e}")
        return False


def _noor_refresh_session():
    """Fetch noorplay homepage to acquire fresh session cookies + CSRF token."""
    global _noor_session_ready, _noor_logged_in
    try:
        r = _noor_session.get(
            "https://www.noorplay.com/ar",
            headers={
                "User-Agent": _NOOR_HEADERS["User-Agent"],
                "Accept-Language": _NOOR_HEADERS["Accept-Language"],
            },
            timeout=15,
        )
        _noor_session_ready = True
        _noor_logged_in = "logout" in r.text.lower() or "تسجيل الخروج" in r.text
        print(f"[noorplay] session refreshed → logged_in={_noor_logged_in}")
    except Exception as e:
        print(f"[noorplay] session refresh failed: {e}")


def _noor_get_xsrf():
    """Extract the decoded XSRF-TOKEN value from session cookies."""
    raw = _noor_session.cookies.get("XSRF-TOKEN", "")
    if raw:
        return urllib.parse.unquote(raw)
    return ""


def noorplay_search(query):
    """Search noorplay.com/ar and return list of result dicts."""
    global _noor_session_ready
    with _noor_session_lock:
        if not _noor_session_ready:
            _noor_refresh_session()
    try:
        headers = dict(_NOOR_HEADERS)
        xsrf = _noor_get_xsrf()
        if xsrf:
            headers["X-XSRF-TOKEN"] = xsrf

        r = _noor_session.get(
            "https://www.noorplay.com/ar/search_result",
        ...
        if "لا توجد نتائج" in html or not html.strip():
            # Session may have expired — try once more with a fresh session
            with _noor_session_lock:
                _noor_refresh_session()
            headers["X-XSRF-TOKEN"] = _noor_get_xsrf()
            r = _noor_session.get(
                "https://www.noorplay.com/ar/search_result",
                params={"term": query, "page": 1},
                headers=headers,
        ...
        urls = re.findall(r'href="(https://www\.noorplay\.com/ar/show/[^"]+)"', html)
        imgs = re.findall(r'data-src="([^"]+)"', html)
        ...
        print(f"[noorplay] search error: {e}")
        return []


def noorplay_login(email, password):
    """Login to NoorPlay and store session. Returns True on success."""
    global _noor_session_ready, _noor_logged_in
    try:
        _noor_session.get(
            "https://www.noorplay.com/ar",
        ...
        xsrf = _noor_get_xsrf()
        r = _noor_session.get(
            "https://www.noorplay.com/ar/auth/login_register",
            headers={"User-Agent": _NOOR_HEADERS["User-Agent"], "Accept-Language": "ar,en;q=0.9"},
            timeout=15,
        )
        page_token = re.search(r"SiteToken\s*=\s*'([^']+)'", r.text)
        token = page_token.group(1) if page_token else xsrf

        resp = _noor_session.post(
            "https://www.noorplay.com/ar/auth/login",
            data={
                "_token": token,
                "identifier": email,
                "password": password,
                "authentication_by": "email",
            },
            headers={
                "User-Agent": _NOOR_HEADERS["User-Agent"],
                "Accept-Language": "ar,en;q=0.9",
                "Referer": "https://www.noorplay.com/ar/auth/login_register",
                "X-Requested-With": "XMLHttpRequest",
                "Accept": "application/json, text/javascript, */*; q=0.01",
                "X-XSRF-TOKEN": urllib.parse.unquote(_noor_session.cookies.get("XSRF-TOKEN", "")),
            },
            timeout=15,
        )
        data = resp.json()
        print(f"[noorplay] login response: {data}")
        if data.get("status") or data.get("success") or (resp.status_code == 200 and "error" not in str(data).lower()):
            _noor_session_ready = True
            _noor_logged_in = True
            _noor_save_cookies()
            print("[noorplay] ✓ logged in and session saved")
            return True, data
        return False, data
    except Exception as e:
        print(f"[noorplay] login error: {e}")
        return False, {"error": str(e)}


def noorplay_get_episodes(show_url):
    """Fetch NoorPlay show page and extract episode list."""
    try:
        r = _noor_session.get(
            show_url,
            headers={"User-Agent": _NOOR_HEADERS["User-Agent"], "Accept-Language": "ar,en;q=0.9"},
        ...
        if not poster_match:
            poster_match = re.search(r'<meta[^>]+content="([^"]+)"[^>]+property="og:image"', html, re.IGNORECASE)
        ...
        ep_raw = re.findall(
            r'href="(https://www\.noorplay\.com/ar/video/(\d+)/[^"]+)"',
            html
        )
        seen = set()
        episodes = []
        for url, vid_id in ep_raw:
            if url not in seen:
                seen.add(url)
                slug = url.rstrip("/").split("/")[-1]
                label = urllib.parse.unquote(slug).replace("-", " ").strip()
                episodes.append({"label": label, "url": url, "source": "noorplay"})
        ...
        print(f"[noorplay] get_episodes error: {e}")
        return {"error": str(e), "episodes": []}


def noorplay_get_video_url(episode_url):
    """Fetch a NoorPlay video page and extract the streaming URL."""
    try:
        r = _noor_session.get(
            episode_url,
            headers={"User-Agent": _NOOR_HEADERS["User-Agent"], "Accept-Language": "ar,en;q=0.9"},
        ...
        if "login_register" in html and "تسجيل الدخول" in html and "play-episode-sec" not in html[:5000]:
            return None, "login_required"

        # Try m3u8 / mp4 direct
        streams = re.findall(r'(https?://[^\s"\'<>]+\.(?:m3u8|mp4)[^\s"\'<>]*)', html)
        streams = [s for s in streams if "thumb" not in s and "poster" not in s and ".jpg" not in s]
        if streams:
            return streams[0], "ok"

        # Mangomolo iframe src
        mango = re.findall(r'src="(https://player\.mangomolo\.com/[^"]+)"', html)
        if mango:
            # Try to resolve mangomolo player
            mp = _noor_session.get(mango[0], headers={"User-Agent": _NOOR_HEADERS["User-Agent"]}, timeout=15)
            mp_html = mp.text
            mp_streams = re.findall(r'(https?://[^\s"\'<>]+\.(?:m3u8|mp4)[^\s"\'<>]*)', mp_html)
            mp_streams = [s for s in mp_streams if "thumb" not in s and ".jpg" not in s]
            if mp_streams:
                return mp_streams[0], "ok"
            return mango[0], "player_url"
...
<p style="margin-top:16px;"><a href="/" style="color:#7ab3f0;">العودة إلى الصفحة الرئيسية وتسجيل الدخول</a></p></div></body></html>"""
    ...
    safe_url   = _html.escape(vid_url)
    proxy_url  = _html.escape("/proxy?url=" + urllib.parse.quote(vid_url, safe=""))
...
<script src="https://pl28987990.profitablecpmratenetwork.com/5f/06/fa/5f06fabd44bfc9ac41931c1adf7bfd92.js"></script>
<style>
...
<div style="display:flex;justify-content:center;align-items:center;background:#111;padding:4px 0;flex-shrink:0;">
  <script>
    atOptions = {{'key':'5f6f3e5b1955414611ce4ccd4a21aa7a','format':'iframe','height':50,'width':320,'params':{{}}}};
  </script>
  <script src="https://www.highperformanceformat.com/5f6f3e5b1955414611ce4ccd4a21aa7a/invoke.js"></script>
</div>
...
<div style="display:flex;justify-content:center;padding:8px;flex-shrink:0;background:#000;">
  <a href="{safe_url}" download target="_blank"
...
<div class="err" id="err">⚠️ تعذّر تشغيل الفيديو في المتصفح.
  <br><a href="{safe_url}" style="color:#7ab3f0;margin-top:8px;display:inline-block;">افتح الرابط مباشرة</a>
</div>
<script>
(function() {{
  var directUrl = "{safe_url}";
  var proxySrc  = "{proxy_url}";
  var vid = document.getElementById('vp');
  var errEl = document.getElementById('err');
  function showErr() {{ errEl.style.display = 'block'; }}
  var isHLS = /m3u8/i.test(directUrl);
  if (isHLS) {{
    if (typeof Hls !== 'undefined' && Hls.isSupported()) {{
      ...
      hls.on(Hls.Events.ERROR, function(ev, d) {{
        if (d.fatal) {{
          hls.destroy();
          var hls2 = new Hls({{ enableWorker: false }});
          hls2.loadSource(directUrl);
          hls2.attachMedia(vid);
          hls2.on(Hls.Events.MANIFEST_PARSED, function() {{ vid.play().catch(function(){{}}); }});
          hls2.on(Hls.Events.ERROR, function(ev2, d2) {{ if (d2.fatal) showErr(); }});
        }}
      }});
    ...
    vid.addEventListener('error', function onErr() {{
      vid.removeEventListener('error', onErr, true);
      vid.src = proxySrc;
      vid.load();
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .badge-noorplay { background: #1a6abf; color: #fff; }
  .badge-alooytv { background: #e50914; color: #fff; }
  .card-noorplay { border: 1px solid #1a6abf22; }
  .noor-login-box { background: #1a1a2e; border: 1px solid #1a6abf; border-radius: 10px; padding: 20px; margin-bottom: 16px; }
  .noor-login-box h3 { color: #1a6abf; margin-bottom: 12px; font-size: 15px; }
  .noor-login-box input { width: 100%; padding: 10px; border-radius: 6px; border: 1px solid #333; background: #111; color: #fff; font-size: 14px; margin-bottom: 8px; }
  .noor-login-box button { background: #1a6abf; color: #fff; border: none; padding: 10px 20px; border-radius: 6px; cursor: pointer; font-size: 14px; width: 100%; }
  .noor-login-box button:hover { background: #1458a0; }
  .noor-status-bar { display:flex; align-items:center; gap:10px; background:#111; border:1px solid #222; border-radius:8px; padding:10px 14px; margin-bottom:14px; font-size:13px; }
  .noor-status-bar .dot { width:9px; height:9px; border-radius:50%; flex-shrink:0; }
  .dot-on { background:#4caf50; box-shadow:0 0 6px #4caf5088; }
  .dot-off { background:#f44336; }
  .noor-status-bar span { flex:1; color:#aaa; }
  .noor-status-bar button { background:#1a6abf; color:#fff; border:none; padding:6px 14px; border-radius:5px; cursor:pointer; font-size:12px; white-space:nowrap; }
  .noor-status-bar button.logout-btn { background:#333; color:#aaa; }
  .noor-status-bar button:hover { opacity:.85; }
  .noor-inline-login { background:#0d1a2e; border:1px solid #1a6abf33; border-radius:8px; padding:14px; margin-bottom:14px; }
  .noor-inline-login h4 { color:#7ab3f0; font-size:13px; margin-bottom:10px; }
  .noor-inline-login .fields { display:flex; gap:8px; flex-wrap:wrap; }
  .noor-inline-login input { flex:1; min-width:140px; padding:8px 10px; border-radius:5px; border:1px solid #333; background:#111; color:#fff; font-size:13px; }
  .noor-inline-login button { background:#1a6abf; color:#fff; border:none; padding:8px 18px; border-radius:5px; cursor:pointer; font-size:13px; white-space:nowrap; }
  .noor-inline-login button:hover { background:#1458a0; }
  .noor-inline-login .hint { color:#555; font-size:11px; margin-top:6px; }
  .ep-btn.noor { border-color: #1a6abf44; color: #7ab3f0; background: #0d1a2e; }
  .ep-btn.noor:hover { background: #1a6abf; color: #fff; border-color: #1a6abf; }
...
function startAdminDl() {
  ...
  const link = `/api/download?url=${encodeURIComponent(url)}&m=${encodeURIComponent(mins)}&k=${encodeURIComponent(key)}&t=video`;
...
function attachPlayer(videoUrl, errMsgId) {
  const vid = document.getElementById('vp');
  if (!vid) return;
  const isHLS = /\\.m3u8|m3u8/i.test(videoUrl);
  const proxySrc = '/proxy?url=' + encodeURIComponent(videoUrl);
  ...
  if (isHLS) {
    if (typeof Hls !== 'undefined' && Hls.isSupported()) {
      ...
      hls.on(Hls.Events.ERROR, (ev, d) => {
        if (d.fatal) {
          hls.destroy();
          const hls2 = new Hls({ enableWorker: false });
          hls2.loadSource(videoUrl);
          hls2.attachMedia(vid);
          hls2.on(Hls.Events.MANIFEST_PARSED, () => vid.play().catch(()=>{}));
          hls2.on(Hls.Events.ERROR, (ev2, d2) => { if (d2.fatal) showErr(); });
        }
      });
    } else if (vid.canPlayType('application/vnd.apple.mpegurl')) {
      vid.src = videoUrl;
    ...
    // Try direct URL first (fastest — no proxy latency)
    vid.src = videoUrl;
    vid.addEventListener('error', function onErr() {
      vid.removeEventListener('error', onErr, true);
  ...
  try {
    ...
    if (data.status === 'Success') {
      ...
      document.getElementById('playerContent').innerHTML = `
        ...
        <div id="vperr" style="display:none;" class="error-box">
          ⚠️ تعذّر التشغيل —
          <a href="${data.video_url}" target="_blank" style="color:#7ab3f0">افتح الرابط مباشرة</a>
        </div>
        <div class="btn-row">
          <button onclick="copyShareLink('${shareLink.replace(/'/g,"\\'")}','share-msg-aloo')" style="background:#1a6a2f;">🔗 رابط المشاركة</button>
          <button onclick="navigator.clipboard.writeText(currentVideoUrl).then(()=>alert('تم النسخ!'))" style="background:#333;">📋 نسخ رابط الفيديو</button>
      ...
      attachPlayer(data.video_url, 'vperr');
    } else {
      document.getElementById('playerContent').innerHTML =
...
def noor_grab_route():
    ...
    if vid_url:
        return jsonify({"status": "Success", "video_url": vid_url, "type": status_str})
...
def grab():
    ...
    row = db_get(ep_url)
    if row and row[0] and row[1] == "ready":
        return jsonify({"status": "Success", "video_url": row[0], "cached": True})

    vid = extract_and_cache(ep_url)
    if vid:
        return jsonify({"status": "Success", "video_url": vid, "cached": False})
    return jsonify({"status": "Failed", "message": "لم يتم العثور على رابط الفيديو"}), 404
