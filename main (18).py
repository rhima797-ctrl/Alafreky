...
def watch_page():
    ...
    if ep_url:
        # check cache first, else extract fresh
        row = db_get(ep_url)
        if row and row[0] and row[1] == "ready":
            vid_url = row[0]
        else:
...
async function doSearch() {
  ...
  try {
    ...
    data.results.forEach(item => {
      ...
      html += `<div class="card" onclick="openSeries('${encodeURIComponent(item.url)}','${item.title.replace(/'/g,"\\'")}')">
